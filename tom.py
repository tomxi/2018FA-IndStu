...
class MelGenerator(keras.utils.Sequence):
    ...
    def _data_from_ids(self, n_ids):
        ...
        x_batch = []
        y_batch = np.multiply(self.Y_true[n_ids], self.Y_mask[n_ids])
        for x_id in x_ids:
...
def construct_crnnL3_smp_tom(input_shape=(None, 128, 1), alpha=0.5, n_classes=20):
    '''
    ...
    p_dynamic = keras.layers.TimeDistributed(p0, name='dynamic/tags')(rnn1)

    p_static = layers.SoftMaxPool(alpha=alpha,
                                  axis=1,
